# Stage 2 Prompt 4 — High-Stakes Execution Interview
...
```text
Prompt Mode: High-Stakes Execution Interview
Interview Threshold: High only
Execution Logging Level: High-Stakes Execution Logging
```

In this mode:

```text
1. Review Stage 1 and identify unresolved execution decisions.
2. Resolve safe low- and medium-stakes execution decisions without interview.
3. Interview the user only for unresolved high-stakes execution decisions.
4. Elevate lower-stakes decisions that materially affect high-stakes outcomes.
5. Line-item log high-stakes decisions, plus elevated lower-stakes decisions.
6. Summarize non-elevated low- and medium-stakes decisions below threshold.
```

Interview the user only for unresolved high-stakes execution decisions unless lower-stakes issues are elevated for materiality.
...
M. Low-Stakes Execution Decisions the AI Will Resolve Without Interview
N. Medium-Stakes Execution Decisions the AI Will Resolve Without Interview
...
| Prompt Mode | High-Stakes Execution Interview |
| Active Logging Level | High-Stakes Execution Logging |
...
Because this mode uses High-Stakes Execution Logging, summarize low- and medium-stakes execution decisions below threshold using the standard below-threshold summary table.
...
Begin with the Stage 2 High-Stakes Execution Interview if unresolved high-stakes decisions remain. Otherwise produce the final artifact.
