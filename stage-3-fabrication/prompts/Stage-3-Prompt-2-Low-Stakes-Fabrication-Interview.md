# Stage 3 Prompt 2 — Low-Stakes Fabrication Interview
...
```text
Prompt Mode: Low-Stakes Fabrication Interview
Interview Threshold: Low and above
Fabrication Logging Level: Low-Stakes Fabrication Logging
```
...
2. Do not ask questions already answered by Stage 2, Stage 1, the user, or the active configuration.
3. Identify unresolved low-, medium-, and high-stakes fabrication decisions.
4. Interview the user for unresolved low-, medium-, and high-stakes fabrication decisions.
5. Do not fabricate final deliverables until the required interview is complete, unless the user authorizes defaults or safe handling.
6. Preserve Stage 1 design decisions and Stage 2 execution-planning decisions.
7. Record all meaningful low-, medium-, and high-stakes fabrication decisions in the Standardized Fabrication Decision Ledger.
```
...
Before producing the final Stage 3 artifact, perform a Stage 3 Low-Stakes Fabrication Interview. Analyze Stage 2 intake, active configuration, execution lock, design lock, fabrication scope, work package execution plan, text-file strategy, binary/external-tool strategy, validation strategy, resolved decisions, unresolved low-stakes decisions, unresolved medium-stakes decisions, unresolved high-stakes decisions, recommended defaults, and interview questions. For each question include Question ID, Stakes Level, Decision Category, Options, Recommended Default, and Why It Matters. After the user answers, apply answers and ask follow-ups for remaining low-, medium-, or high-stakes issues unless the user authorizes defaults or safe handling.
...
| Prompt Mode | Low-Stakes Fabrication Interview |
| Active Logging Level | Low-Stakes Fabrication Logging |
...
Because this mode uses Low-Stakes Fabrication Logging, line-item log all meaningful low-, medium-, and high-stakes fabrication decisions. The Below-Threshold Fabrication Decision Summary should state: Not applicable.
...
Begin by performing the Stage 3 Low-Stakes Fabrication Interview. Do not fabricate final deliverables until the required interview is complete, unless the user explicitly instructs you to proceed with defaults, placeholders, review-gated handling, blocked-item handling, external-tool-required handling, exclusions, or not-applicable determinations.
