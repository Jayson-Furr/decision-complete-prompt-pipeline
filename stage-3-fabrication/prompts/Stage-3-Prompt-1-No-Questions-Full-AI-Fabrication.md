# Stage 3 Prompt 1 — No Questions — Full AI Fabrication
...
```text
Prompt Mode: No Questions — Full AI Fabrication
Interview Threshold: None
Fabrication Logging Level: Full AI Fabrication Logging
```
...
2. Proceed immediately using the Stage 2 artifact, Stage 1 design authority, configuration files, and available fabrication context.
3. Make safe low-, medium-, and high-stakes fabrication decisions where allowed.
4. Use placeholders, review gates, blockers, external-tool-required status, exclusions, or not-applicable determinations when a decision cannot be safely made.
5. Preserve Stage 1 design decisions and Stage 2 execution-planning decisions.
6. Record all meaningful low-, medium-, and high-stakes fabrication decisions in the Standardized Fabrication Decision Ledger.
7. Produce a 100% fabrication-complete Stage 3 artifact, or accurately classify the artifact as not yet fabrication-complete with blockers.
```
...
No interview is performed in this mode. Create a Stage 3 intake summary internally, resolve decisions using safe defaults and controlled handling, then proceed directly to fabrication. If any high-stakes item cannot be safely fabricated, preserve it through blocked, review-gated, external-authorization-required, external-tool-required, excluded, or not-applicable handling rather than asking questions.
...
| Prompt Mode | No Questions — Full AI Fabrication |
| Active Logging Level | Full AI Fabrication Logging |
...
Because this mode uses Full AI Fabrication Logging, line-item log all meaningful low-, medium-, and high-stakes fabrication decisions. The Below-Threshold Fabrication Decision Summary should state: Not applicable.
...
Produce the final Stage 3 artifact now. Do not ask clarifying questions. Execute all ready Stage 2 work packages that can be safely executed. For text-based files, provide complete file contents or create actual files when the environment supports file creation. For binary or external-tool-dependent assets, do not pretend the asset exists unless it was actually created; mark it accurately.
