# Stage 4 Prompt 1 — No Questions — Full AI Validation and Packaging
...
```text
Prompt Mode: No Questions — Full AI Validation and Packaging
Interview Threshold: None
Finalization Logging Level: Full AI Finalization Logging
```
...
2. Proceed immediately using the Stage 3 artifact, Stage 2 execution authority, Stage 1 design authority, configuration files, and available finalization context.
3. Validate every fabricated artifact that can be validated in the current environment.
4. Perform only allowed repairs and log every repair.
5. Preserve placeholders, review-gated items, blocked items, and external-tool-required items.
6. Do not invent missing design, execution, fabrication, rights, credential, authorization, or approval information.
...
No interview is performed in this mode. Create a Stage 4 intake and finalization analysis internally, resolve decisions using safe defaults and controlled handling, then proceed directly to validation, allowed repair, packaging, and final handoff preparation. If a high-stakes item cannot be safely finalized, preserve it through blocked, review-gated, repair-gated, external-validation-required, external-authorization-required, professional-review-required, excluded, or not-applicable handling.
...
| Prompt Mode | No Questions — Full AI Validation and Packaging |
| Active Logging Level | Full AI Finalization Logging |
...
Because this mode uses Full AI Finalization Logging, line-item log all meaningful low-, medium-, and high-stakes finalization decisions. The Below-Threshold Finalization Decision Summary should state: Not applicable.
...
Produce the final Stage 4 artifact now. Do not ask clarifying questions. Validate all Stage 3 fabricated artifacts that can be validated safely in the current environment. Perform only allowed repairs. Log every repair. Do not deploy, publish, submit, release, use credentials, perform destructive operations, claim approvals, or resolve review-gated items without explicit authorization.
