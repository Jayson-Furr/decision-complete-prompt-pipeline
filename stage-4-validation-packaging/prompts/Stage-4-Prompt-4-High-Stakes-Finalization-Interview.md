# Stage 4 Prompt 4 — High-Stakes Finalization Interview
...
```text
Prompt Mode: High-Stakes Finalization Interview
Interview Threshold: High only
Finalization Logging Level: High-Stakes Finalization Logging
```
...
2. Make safe low-stakes finalization decisions without asking the user.
3. Make safe medium-stakes finalization decisions without asking the user.
4. Interview the user only for unresolved high-stakes finalization decisions.
5. Use controlled placeholders, review-gated handling, repair-gated handling, blocked-item handling, external-validation handling, exclusions, or not-applicable determinations for high-stakes finalization issues that cannot safely be resolved.
6. Validate every fabricated artifact that can be validated in the current environment.
7. Perform only allowed repairs and log every repair.
8. Line-item log high-stakes decisions and summarize low- and medium-stakes decisions below threshold.
```
...
Before final validation, perform a Stage 4 High-Stakes Finalization Interview only if unresolved high-stakes finalization decisions remain. Analyze Stage 3 intake, active configuration, design lock, execution lock, fabrication lock, validation scope, repair permission policy, packaging strategy, final artifact inclusion strategy, final QA strategy, approval and release authorization handling, resolved decisions, low-stakes decisions the AI will resolve, medium-stakes decisions the AI will resolve, unresolved high-stakes decisions, recommended high-stakes safe handling options, and interview questions. Do not ask low- or medium-stakes questions unless elevated for materiality because they directly affect a high-stakes issue. If no unresolved high-stakes questions exist, proceed directly to validation, repair, packaging, and final handoff preparation.
...
| Prompt Mode | High-Stakes Finalization Interview |
| Active Logging Level | High-Stakes Finalization Logging |
...
Because this mode uses High-Stakes Finalization Logging, line-item log high-stakes finalization decisions. Summarize non-elevated low- and medium-stakes finalization decisions in the Below-Threshold Finalization Decision Summary.
...
Begin by performing the Stage 4 High-Stakes Finalization Interview if unresolved high-stakes finalization decisions remain. Do not ask low- or medium-stakes finalization questions unless they are elevated for materiality because they directly affect a high-stakes issue. If no unresolved high-stakes finalization decisions remain after context review, proceed directly to validation, repair, packaging, and final handoff preparation.
