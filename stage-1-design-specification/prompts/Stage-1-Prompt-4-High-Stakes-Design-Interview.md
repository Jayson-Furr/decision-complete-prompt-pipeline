# Stage 1 Prompt 4 — High-Stakes Design Interview
...
```text
Prompt Mode: High-Stakes Design Interview
Interview Threshold: High only
Design Logging Level: High-Stakes Design Logging
```
...
2. Resolve safe low- and medium-stakes design decisions without interview.
3. Interview the user only for unresolved high-stakes design decisions.
...
Interview the user only for unresolved high-stakes design decisions unless lower-stakes issues are elevated for materiality.
...
N. Medium-Stakes Design Decisions the AI Will Resolve Without Interview
...
| Prompt Mode | High-Stakes Design Interview |
| Active Logging Level | High-Stakes Design Logging |
...
Because this mode uses High-Stakes Design Logging, summarize low- and medium-stakes design decisions below threshold using the standard below-threshold summary table.
...
Begin with the Stage 1 High-Stakes Design Interview if unresolved high-stakes decisions remain. Otherwise produce the final artifact.
